...
- Поддерживаем IDN-домены: можно указывать `https://медвежата.рф` — внутри он будет сохранён в punycode (`https://xn--...`).
- Если у вас много поддоменов (городов и т.п.), добавьте один origin с подстановкой поддомена: `https://*.медвежата.рф` — CORS будет работать для всех поддоменов этого домена.
